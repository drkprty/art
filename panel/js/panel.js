...
async function contentApiRequest(path,{method='GET',body=null,headers={}}={}){
  if(!contentApiReady()) throw new Error('GitHub upload Worker is not configured yet. Paste its URL in Site details.');
  const user=auth.currentUser;
  if(!user) throw new Error('You must be signed in.');
  const idToken=await user.getIdToken();
  const base=getContentApiBase().replace(/\/+$/,'');
...
onAuthStateChanged(auth, async user => {
  ...
  if(!contentApiReady()) showConnectionBanner('GitHub image uploads are not configured yet. Paste the Cloudflare Worker URL in Site details.');
